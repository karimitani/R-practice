#Import the Data
getwd()
setwd("Users/karimitani/Desktop/Yallaaaaa/R A-Z")
...
q <- p + 
  geom_jitter(aes(sizesize=Budget...mill.,colour=Studio))+
...
p + 
  geom_jitter(aes(size=Budget...mill., _=Studio)) + 
  geom_boxplot(alpha_=0.7) 
...
q <- p + 
  geom_jitter(aes(_size=Budget...mill., colour=Studio)) + 
  geom_boxplot(alpha = 0.7, outlier.colour = NA) 
q

#Non-data ink
q <- q +
  xlabel("Genre") + #x axis title
  ylabel("Gross % US") + #y axis title
...
q <- q + 
  theme(
    ...
    #Axes titles:
    Axes.title = element_text(colour="Blue", size=30),
    Axes.title = element_text(colour="Blue", size=30),
